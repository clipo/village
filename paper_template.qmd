...
# Load the data
data <- read.csv("data/sample_c14_data.csv")
...
summary_table <- data %>%
  ...
  summarise(
    `N Dates` = n(),
    `Mean Age (BP)` = round(mean(age), 0),
    `SD (BP)` = round(sd(age), 0),
    `Age Range (BP)` = paste0(min(age), "-", max(age)),
    `Mean Error (±)` = round(mean(error), 0),
...
We considered three model variants representing different temporal hypotheses:
