...
The question of contemporaneity—whether archaeological sites or deposits were occupied simultaneously—is central to understanding past human societies [@bronkramsey2009]. Determining which sites were occupied at the same time allows archaeologists to reconstruct interaction networks, trace the spread of cultural innovations, identify migration events, and understand regional settlement patterns [@contreras2016]. However, establishing contemporaneity from radiocarbon dates is challenging due to the probabilistic nature of radiocarbon calibration, measurement uncertainty, and the need to account for occupation span rather than single-point events [].

Traditional approaches to assessing contemporaneity have often relied on visual comparison of calibrated date ranges or informal overlap assessments []. While informative, these methods lack formal statistical rigor and do not provide probabilistic estimates of temporal relationships. Bayesian chronological modeling offers a powerful alternative by combining radiocarbon measurements with stratigraphic information and prior knowledge to generate posterior probability distributions for events of interest [@buck1996].

## Study Area and Archaeological Context

This study focuses on five archaeological sites attributed to the Late Woodland period (ca. 1000-300 BP) in the Upper Susquehanna River Valley and surrounding regions of New York State. This period is characterized by the adoption and intensification of maize agriculture, population aggregation into larger villages, and the development of distinctive ceramic traditions []. Understanding the temporal relationships between Late Woodland sites is crucial for interpreting the spread of agricultural practices, the formation of proto-Iroquoian settlement patterns, and potential population movements during this transformative period.
...
All radiocarbon dates in this study were obtained from cultigens (maize, beans, and squash), which have the advantage of being short-lived samples that closely reflect the actual time of occupation [].
...
Cultigen samples were selected deliberately because they represent short-lived organisms that provide direct dates on human activity rather than old-wood effects common with charcoal samples []. All samples were recovered from secure archaeological contexts (storage pits, house floors, middens) and processed by reputable radiocarbon laboratories (Beta Analytic, University of Arizona AMS Laboratory).

### Data Quality and Outlier Assessment

Prior to analysis, we evaluated the radiocarbon dataset for potential outliers using established criteria []. We examined:
...
library(ggplot2)
img <- png::readPNG("output/sample_analysis/01_calibrated_dates.png")
ggplot() +
...
- `cmdstanr` v0.8.1: Interface to Stan []
- `posterior` v1.6.0: Posterior distribution analysis []
- `bayesplot` v1.11.1: MCMC diagnostics and visualization [@gabry2019]
- `loo` v2.8.0: Model comparison []
...
img <- png::readPNG("output/sample_analysis/02_occupation_boundaries.png")
ggplot() +
  annotation_custom(grid::rasterGrob(img, interpolate=TRUE),
                    xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf) +
  theme_void()
...
img <- png::readPNG("output/sample_analysis/03_temporal_overlap.png")
ggplot() +
  annotation_custom(grid::rasterGrob(img, interpolate=TRUE),
                    xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf) +
  theme_void()
...
# Move site column to front and use as row names
...
#| fig-cap: "Heatmap of pairwise contemporaneity probabilities. Green indicates high probability of temporal overlap (near 1.0), while dark red indicates low probability (near 0.0). The strong clustering of Broome Tech, Chenango Point, and Thomas Luckey is evident, as is the temporal separation of CCE Site from Hill Creek."
#| echo: false
#| warning: false
#| fig-height: 5
#| fig-width: 6
...
img <- png::readPNG("output/sample_analysis/04_overlap_matrix.png")
ggplot() +
  annotation_custom(grid::rasterGrob(img, interpolate=TRUE),
                    xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf) +
  theme_void()
...
Figure 6 presents an integrated visualization showing occupation timelines with contemporaneity groupings based on overlap probabilities.

```{r}
#| label: fig-contemporaneity-timeline
#| fig-cap: "Integrated contemporaneity timeline showing occupation spans (lines) with 68% (dark bands) and 95% (light bands) credible intervals. Sites are colored by contemporaneity group (>95% overlap probability threshold). The visualization reveals two main temporal clusters: an early-to-intermediate group (Hill Creek, Thomas Luckey, Broome Tech, Chenango Point) and a late isolated occupation (CCE Site). Sites are ordered chronologically from earliest (top) to latest (bottom) based on median start dates."
#| echo: false
#| warning: false
#| fig-width: 10
#| fig-height: 6
...
img <- png::readPNG("output/sample_analysis/05b_contemporaneity_timeline.png")
ggplot() +
  annotation_custom(grid::rasterGrob(img, interpolate=TRUE),
                    xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf) +
  theme_void()
...
The contemporaneity timeline clearly visualizes the temporal relationships identified in the overlap analysis. Sites sharing the same color belong to contemporaneity groups with >95% overlap probability. The substantial temporal overlap between Broome Tech, Chenango Point, and Thomas Luckey (377-524 years) is evident in their extensive band overlap and shared group membership.
...
Figure 7 shows posterior predictive checks comparing observed radiocarbon dates to model predictions. For each date, we simulated 100 replicate observations from the posterior predictive distribution and compared them to the actual measurement.
...
img <- png::readPNG("output/sample_analysis/05_posterior_predictive.png")
ggplot() +
  annotation_custom(grid::rasterGrob(img, interpolate=TRUE),
                    xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf) +
  theme_void()
...
Beyond pairwise overlap probabilities, we conducted targeted hypothesis tests to quantify specific temporal relationships.

### Overlap Duration Estimates

Table 5 presents the estimated overlap durations (in years) for site pairs with high contemporaneity probability. These estimates provide quantitative assessments of the temporal extent of potential interaction periods.

```{r}
#| label: tbl-overlap-durations
#| tbl-cap: "Estimated overlap durations (years) for site pairs with high contemporaneity probability (p > 0.95). Values represent posterior means with 95% credible intervals."
#| echo: false
#| warning: false

overlap_durations <- read_csv("output/hypothesis_tests/pairwise_overlap_durations.csv", show_col_types = FALSE)

overlap_durations_display <- overlap_durations %>%
  filter(prob_any_overlap > 0.95) %>%
  mutate(
    `Site Pair` = paste(site_1, "-", site_2),
    `Mean Overlap (years)` = round(overlap_mean, 0),
    `95% CI` = paste0("[", round(overlap_95ci_lower, 0), ", ", round(overlap_95ci_upper, 0), "]"),
    `P(overlap)` = round(prob_any_overlap, 3)
  ) %>%
  select(`Site Pair`, `Mean Overlap (years)`, `95% CI`, `P(overlap)`)

kable(overlap_durations_display, align = "lccc")
```

The three contemporaneous sites (Broome Tech, Chenango Point, Thomas Luckey) show substantial overlap durations ranging from 377 to 524 years, with high statistical certainty (95% credible intervals all exceeding 275 years). This extended temporal overlap strongly supports sustained contemporaneous occupation and potential cultural interaction.

### Temporal Precedence

We tested directional precedence relationships to identify cases where one site definitively precedes another with no temporal overlap. Table 6 shows relationships with precedence probability exceeding 0.5.

```{r}
#| label: tbl-precedence
#| tbl-cap: "Temporal precedence relationships with probability > 0.5. Gap duration represents the time between end of earlier site and start of later site."
#| echo: false
#| warning: false

precedence <- read_csv("output/hypothesis_tests/precedence_tests.csv", show_col_types = FALSE)

precedence_display <- precedence %>%
  filter(prob_precedence > 0.5) %>%
  arrange(desc(prob_precedence)) %>%
  mutate(
    `Sequence` = paste(earlier_site, "→", later_site),
    `P(precedence)` = round(prob_precedence, 3),
    `Gap (years)` = round(gap_mean, 0),
    `95% CI` = paste0("[", round(gap_95ci_lower, 0), ", ", round(gap_95ci_upper, 0), "]")
  ) %>%
  select(`Sequence`, `P(precedence)`, `Gap (years)`, `95% CI`)

if (nrow(precedence_display) > 0) {
  kable(precedence_display, align = "lccc")
} else {
  cat("No strong precedence relationships detected (all probabilities < 0.5)\n")
}
```

Hill Creek shows strong evidence of preceding CCE Site (p = 0.995), with a gap of approximately 175 years (95% CI: [61, 219] years). This temporal separation indicates these sites represent distinct occupation phases rather than a continuous settlement pattern.

### Temporal Ordering Hypotheses

We evaluated five competing hypotheses about the overall temporal ordering of sites. Table 7 presents the posterior probabilities for each ordering scenario.

```{r}
#| label: tbl-ordering-hypotheses
#| tbl-cap: "Posterior probabilities for alternative temporal ordering hypotheses. Relative support indicates the probability of each hypothesis relative to the set tested."
#| echo: false
#| warning: false

ordering <- read_csv("output/hypothesis_tests/ordering_hypotheses.csv", show_col_types = FALSE)
...
ordering_display <- ordering %>%
  mutate(
    ...
    `Relative Support` = paste0(round(relative_support * 100, 1), "%")
  ) %>%
  select(Hypothesis, Sequence, Probability, `Relative Support`)

kable(ordering_display, align = "llcc")
```

The most supported temporal sequence is Hill Creek → Thomas Luckey → Broome Tech → Chenango Point → CCE Site (59% relative support), though absolute probabilities for strict orderings are low due to the substantial overlap among the middle three sites. This reflects the model's appropriate uncertainty about precise ordering when sites have contemporaneous occupations.
...
**A Regional Settlement System**: The three sites may have functioned as a network of contemporaneous villages within an integrated settlement system []. In Late Woodland contexts, such systems often featured village clusters connected by kinship ties, trade networks, and shared ceremonial practices []. The similar occupation spans and agricultural emphasis (all sites yielded abundant maize) support this interpretation.

**Cultural Affinity**: Contemporaneous occupation alone does not prove direct interaction, but the timing coincides with the widespread adoption of Northern Flint maize varieties in the Northeast [] and the formation of proto-Iroquoian settlement patterns []. The sites may represent communities participating in shared cultural traditions and technological innovations.

**Demographic Aggregation**: The period around 500-700 BP (cal AD 1250-1450) corresponds to a time of increasing village size and population aggregation in the region []. The contemporaneous occupation of multiple large villages may reflect regional population growth and the formation of more complex social organization.
...
**Occupation Modeling**: By explicitly modeling occupation boundaries rather than treating dates as instantaneous events, we better represent the archaeological reality that sites were occupied over extended periods [@bronkramsey2009].
...
3. **Summed probability distributions (SPD)**: Useful for detecting population trends [] but not designed for site-to-site contemporaneity assessment
...
**Dispersed Village Pattern**: Rather than a single large settlement, the region supported multiple contemporaneous villages. This dispersed pattern is characteristic of Late Woodland settlement organization [; ].
...
**Agricultural Landscape**: The abundant maize remains at all three sites indicate intensive agriculture during their contemporaneous occupation. This suggests a well-established agricultural landscape with cleared fields and storage infrastructure [].

**Proto-Iroquoian Development**: The temporal clustering around 500-700 BP coincides with the formation of larger, more permanent villages in the Iroquoian culture area []. These sites may represent an intermediate stage in the development from Late Woodland to Contact period settlement patterns.
...
Despite these biases, cultigen dates provide the most reliable chronological information for Late Woodland agricultural villages [].
